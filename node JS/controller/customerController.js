...
exports.PendingPackages=(req,res,next)=>{
    ...
    Package.findAll({where:{send_userName:userName,status:"Under review"}})
    .then((result) => {
...
exports.notPendingPackages=(req,res,next)=>{
    const userName=req.query.userName;
    Package.findAll({where:{send_userName:userName,status:{ [Sequelize.Op.not]: "Under review" }}})
    .then((result) => {
        res.status(200).json({message:'done',result:result}); 
    }).catch((err) => {
        res.status(500).json({message:'failed'}); 
...
exports.postDeletePackage=(req,res,next)=>{
    ...
    Package.destroy({where:{send_userName:customerUserName,packageId:packageId,status:"Under review"}})
    .then((result) => {
...
exports.postEditPackageLocation=(req,res,next)=>{
    ...
    Package.update(
        ...
        {where:{rec_userName:customerUserName,packageId:packageId,status:"Under review"}})
    .then((result) => {
            ...
            const emailTemplate = (recName) => {
                return `
                  <html>
                    <body>
                      <p>Hello ${recName},</p>
                      <p>${result2.Fname+" "+result2.Lname} has update the package(${packageId}) detailes, 
                      you can track the status of the package by downloading Package4U application and writing the attached package number.</p>
...
exports.getPackageState=(req,res,next)=>{
    const packageId=req.query.packageId;
    Package.findOne({include:[{model:User,as:"driver"}],
        attributes: ['status', 'driverComment', 'receiveDate', 'deliverDate','driver_userName'],
        where:{packageId:packageId}})
        .then((result) => {
            if(result==null)
            res.status(200).json({message:'invalid id'}); 
        else{
        Driver.findOne({where:{userUserName:result.driver_userName}}).then((result2) => {
            res.status(200).json({message:'done',result:result,driver:result2}); 
        }).catch((err) => {
            console.log(err)
            res.status(500).json({message:'failed'}); 
        });
