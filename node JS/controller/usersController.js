...
const user = Customer.belongsTo(User, {
  as: "user",
  constraints: true,
  onDelete: "CASCADE",
});
          ...
          Token.findOne({ where: { email: email } })
            .then((result) => {
              ...
              const info = trans.sendMail({
                ...
                to: email,
                subject:
                  "Password Reset Verification Code for Package4U account",
      ...
      .then((result) => {
        if (result[0] === 1) {
          const currentFileName =
            path.join(__dirname, "..", "user_images", "/") + oldUserName + url; // Replace with the actual current file name
          const newFileName =
            path.join(__dirname, "..", "user_images", "/") + userName + url;
...
exports.postChangePassword = (req, res, next) => {
  ...
  if (!error.isEmpty()) {
    ...
    if (oldPassword === password)
      res.status(200).json({ message: "please choose a different password." });
    else
      User.update(
        { password: password },
        {
        ...
        .then((result) => {
          if (result[0] === 1) res.status(200).json({ message: "done" });
            ...
            res
              .status(200)
              .json({
                message: "The old password is incorrect, please verify it.",
              });
        })
        .catch((err) => {
          console.log(err);
