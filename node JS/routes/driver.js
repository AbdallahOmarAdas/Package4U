  ...
  User.findOne({
    ...
    .then((result) => {
      if (result) {
        ...
        res.status(403).json({
          message: "failed",
          message2: "This operation is allowed only for driver",
        });
      }
    })
...
router.post("/getDeliverdDriver", driverController.getDeliverdDriver);
router.post(
  "/getPreparePackageDriver",
...
router.post(
  "/compleatePackageDriver",
  driverController.postCompleatePackageDriver
);
router.post(
  "/RejectWorkOnPackageDriver",
...
router.get("/driverLocation", driverController.GetDriverLocation);
router.post(
    "/EditLocation",
    driverController.PostEditLocation
  );
